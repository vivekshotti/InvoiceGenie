<script>
  export default {
      name: 'HomeInfo',
  } 
</script>

<template>

    <div class="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
      Your One Stop <p class="underline underline-offset-8 decoration-indigo-500">Invoicing Solution.</p><br/>
      🤯<br/>
    </div>
    <p class="leading-8 mt-3 text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
      InvoiceGenie lets you create invoices in a flick of a wind.<br/>
      It helps you monitor all your invoices, update their statuses, and track it efficiently.<br/>
      <br/>
      YOUR responsibilities are now OURS. 
    </p>

</template>

<style scoped>

</style>
